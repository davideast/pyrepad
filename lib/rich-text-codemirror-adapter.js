...
firepad.RichTextCodeMirrorAdapter = (function () {
  ...
  RichTextCodeMirrorAdapter.prototype.setOtherCursor = function (cursor, color, clientId) {
    ...
    if (cursor.position === cursor.selectionEnd) {
      // show Google Docs & Figma style multiplayer cursor with name tooltip
      var cursorCoords = this.cm.cursorCoords(cursorPos);
      var height = (cursorCoords.bottom - cursorCoords.top) * 0.95;

      var cursorEl = document.createElement('span');
      cursorEl.className = 'other-client firepad-client-cursor';
      cursorEl.setAttribute('data-clientid', clientId);
      cursorEl.style.height = (height > 0 ? height : 21) + 'px';
      cursorEl.style.verticalAlign = '-5px';
      cursorEl.style.zIndex = '15';

      var caretEl = document.createElement('span');
      caretEl.className = 'firepad-client-caret';
      caretEl.style.backgroundColor = color;
      caretEl.style.height = '100%';
      cursorEl.appendChild(caretEl);

      var tooltipEl = document.createElement('div');
      tooltipEl.className = 'firepad-client-tooltip firepad-tooltip-visible';
      tooltipEl.style.backgroundColor = color;
      tooltipEl.innerText = clientId || 'Collaborator';
      cursorEl.appendChild(tooltipEl);

      var fadeTimeout = setTimeout(function() {
        if (tooltipEl && tooltipEl.parentElement) {
          tooltipEl.className = 'firepad-client-tooltip firepad-tooltip-hidden';
        }
      }, 3500);

      cursorEl.addEventListener('mouseenter', function() {
