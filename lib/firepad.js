...
firepad.Firepad = (function(global) {
  ...
  function Firepad(ref, place, options) {
    ...
    this.firebaseAdapter_ = this.getOption('syncAdapter', null) || ((this.getOption('useSyncSeam', false) || (ref && typeof ref.toString === 'function' && ref.toString().indexOf('pyric-sandbox://seam') >= 0)) && firepad.SyncSeam ? new firepad.SyncSeam.PyricSandboxAdapter(ref, userId, userColor) : new FirebaseAdapter(ref, userId, userColor));
    if (this.codeMirror_) {
  ...
  Firepad.prototype.dispose = function() {
    ...
    if (this.firepadWrapper_ && editorWrapper) {
      if (editorWrapper.parentNode === this.firepadWrapper_) {
