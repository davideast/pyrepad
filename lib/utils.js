...
firepad.utils.makeEventEmitter = function(clazz, opt_allowedEVents) {
  ...
  clazz.prototype.validateEventType_ = function(eventType) {
    if (this.allowedEvents_) {
      var isAgentive = eventType === 'agentive';
      var isPresence = eventType === 'presence';
      var isWorkerSync = eventType === 'worker_sync';
      var isReconnected = eventType === 'reconnected';
      var allowed = isAgentive || isPresence || isWorkerSync || isReconnected;
      for(var i = 0; i < this.allowedEvents_.length && !allowed; i++) {
        var isMatch = this.allowedEvents_[i] === eventType;
        if (isMatch) {
          allowed = true;
        }
      }
      var isNotAllowed = !allowed;
      if (isNotAllowed) {
